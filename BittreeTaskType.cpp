...
void
BittreeNode::populate_programs(vector<Rule> *rules, CanvasAndBittreeProgram *canvas, int next_child, vector<int> *path,
    ...
    if(subtree_sum == 0)
    {
        ...
        else
        {
            ...
            {
                populate_programs(rules, canvas, next_child + 1, path, all_programs, cost_remaining);
            }
        }
    }
    else {
        if (next_child == -1 || node_type == leaf_node) {
            ...
            for (int i = 0; i < rules->size(); i++) {
                all_programs->push_back(canvas->produce(rules->at(i), path));
            }
        } else {
            if (node_type == internal_node) {
                assert(next_child >= 0);
                if (next_child == children.size()){
                    ...
                    if(children[next_child]->subtree_sum >= 1) {
                        ...
                        for (int new_program_id = 0; new_program_id < programs_with_next_primitive.size(); new_program_id++) {
                            populate_programs(rules, programs_with_next_primitive[new_program_id], next_child + 1, path,
                                              all_programs, 0);
                        }
                    }
                    else
                    {
                        ...
                        {
                            populate_programs(rules, canvas, next_child + 1, path, all_programs, 0);
                        }
                    }
                }
            } else {
