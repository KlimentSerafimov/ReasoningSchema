...
void
BittreeNode::populate_programs(vector<Rule> *rules, CanvasAndBittreeProgram *canvas, int next_child,
                               vector<int> *path, vector<CanvasAndBittreeProgram*> * all_programs) const {
...
int CanvasAndBittreeProgram::get_cost() {
    int cost = 0;
    for(int j = 0;j<code.size();j++) {
        cost += rule_cost[code[j].get_rule()];
    }
    return cost;
}
