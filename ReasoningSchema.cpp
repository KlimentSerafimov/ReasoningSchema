...
ReasoningSchema::ReasoningSchema(int num_inputs, int generalization_mask, int training_mask, vector<MetaExample> meta_examples) {
    ...
    {
        assert(compact_poset.append(meta_examples[i]));
    }
}
        ...
        if(local_partial_function.partition_size() < function_size) {
            ...
            for (int k = 0; k < mask_ids.size(); k++) {
                ...
                MetaExample first_partial_meta_example = MetaExample(
                        ...
                        first_local_partition, first_mask);
                MetaExample second_partial_meta_example = MetaExample(
                        num_inputs,
                        train_meta_examples[i].generalization.total_function,
                        second_local_partition, second_mask);
...
//                cout << first_partial_meta_example.to_string() << " "<< second_partial_meta_example.to_string() << endl;
                ...
                if(first_mask != first_local_partition) {
                    if (!subdomain_poset[mask_ids[k]].first.append(first_partial_meta_example)) {
                    ...
                    else
                    {
                        ...
                        {
                            if (!subdomain_poset[mask_ids[k]].second.append(second_partial_meta_example)) {
...
//                assert(remainders[j].append(local_meta_example));
//            }
//        }
...
//    vector<MetaExample> remainder_meta_examples = remainders[best_subdomain].get_all_meta_examples_without_duplicates();
    ...
    factor_schemas = make_pair(
            new ReasoningSchema(num_inputs, masks[best_subdomain].first, training_masks[best_subdomain].first, subdomain_poset[best_subdomain].first.get_necessary_meta_examples()),
            new ReasoningSchema(num_inputs, masks[best_subdomain].second, training_masks[best_subdomain].second, subdomain_poset[best_subdomain].second.get_necessary_meta_examples())
            );
